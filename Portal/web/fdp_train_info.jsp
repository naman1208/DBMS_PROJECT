...
<%
    ...
    if(request.getParameter("b1")!=null)
    {
        ...
        {
            stmt = con.prepareStatement("select * from ExpertLecture where TID=(select TID from Teacher where Name = ?) order by YearStart");
        ...
        else
        {
            ...
            stmt = con.prepareStatement("select * from ExpertLecture where TID=(select TID from Teacher where Name = ?) and"
                    + " YearStart >= ? and YearStart<= ? order by YearStart");
            stmt.setString(1,tname);
            stmt.setString(2,s);
            stmt.setString(3, e);
    ...
    <body>
        ...
        <div class="main">
                ...
                <% if(p==1){
                    if(m==1){ 
                %>
                <h3>Total participation in  FDPs and STTPs</h3> <% } else if(b==1) { %>
                <h3>Participation in FDP in year(<%=s %> - <%=e %>)</h3> <% }if(l==1) { %>
                <h3>by <%= tname %></h3> <% } %>
                <table>
                <tr>
                    <th>S.no</th>
                ...
                <tr>
                    ...
                    <td><%= rs2.getString(5) %> to <%=rs2.getString(6) %></td><% if(l!=1) {%>
